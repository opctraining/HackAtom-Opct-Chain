...
import { Exercise } from "./module/types/exercise/exercise"
import { Params } from "./module/types/exercise/params"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: hackatomopctchain.exercise initialized!')
		...
		async sendMsgCreateExercise({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateExercise(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateExercise:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgCreateExercise:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		
		async MsgCreateExercise({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateExercise(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgCreateExercise:Create Could not create message: ' + e.message)
				}
			}
		},
